...
describe("genomics parity: microarray input", () => {
  ...
  // Regression: a real AncestryDNA download has an UNCOMMENTED column-name row
  // and a comment block that mentions the word "(genotype)". Provider detection
  // used to see only the commented lines, match the prose, and route the whole
  // file through the 23andMe parser - which reads one combined genotype column
  // instead of two allele columns, so every row was dropped and the component
  // reported zero variants. This pins the ChatGPT route to the portal's fix.
  it("parses a real AncestryDNA header with an uncommented column row", async () => {
    const file = fakeFile(
      "AncestryDNA.txt",
      [
        "#AncestryDNA raw data download",
        "#This file was generated by AncestryDNA at: 07/29/2019 14:50:41 UTC",
        ...
        "#corresponds to a SNP. Columns four and five contain the two alleles",
        "#observed at this SNP (genotype).  The genotype is reported on the forward",
        "#(+) strand with respect to the human reference.",
        "rsid\tchromosome\tposition\tallele1\tallele2",
        "rs328\t8\t19819724\tA\tG",
        "rs2230021\t12\t112241766\tG\tA",
      ].join("\n"),
    );
    ...
    expect(result.providerLabel).toBe("Ancestry");
    expect(result.genomeBuild).toBeNull();
    // A misdetection yields {} here, which is the exact symptom of the bug.
    expect(result.snps).toEqual({ rs328: "AG", rs671: "AG" });
    expect(result.coverage).toEqual({ matched: 2, total: 3 });
  });
});
