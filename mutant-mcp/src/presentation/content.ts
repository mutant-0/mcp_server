...
function detailsContent(data: JsonObject): string {
  ...
  if (whyRanked) parts.push(whyRanked);

  const patterns = asList(explanation?.top_contributing_patterns)
  ...
  if (patternNames.length > 0) {
    parts.push(`The strongest contributing patterns were ${joinNatural(patternNames)}.`);
  }
