...
export const SERVER_INSTRUCTIONS = [
  ...
  "Routing: call get_analysis_status first when readiness is unknown. When DNA is missing, call show_dna_import in the same turn. When the analysis is ready and the user opens Mutant or asks for a general overview, call show_analysis_overview in the same turn and let its card show the accessible findings and hints; do not replace it with a prose recap. The card reads status and context itself. Do not narrate status after rendering the card.",
